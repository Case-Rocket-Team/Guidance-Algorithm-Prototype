...
impl Point<isize> {
    pub fn point_new(x: isize, y: isize, tx: isize, ty: isize, gas: isize) -> Point<isize> {
        Point {
            coords: (x, y),
            tang: (tx, ty),
            gas,
...
impl HyperParams {
    pub fn hp_new(num_points: usize, min_turn: isize, max_curve: isize, max_search: isize, margin: isize) -> HyperParams {
        HyperParams {
            num_points,
            min_turn,
            max_curve,
...
impl RRTWrapper {
    pub fn rrt_new(start: Point<isize>, goal: Point<isize>, hp: HyperParams) -> Self {
    ...
    fn rrt(&mut self) -> Option<usize> {
        if let Some(pnt_node) = self.heap.pop() {
            ...
            let dist_remain = pnt.gas - pnt.dist_2_goal.unwrap_or(0);
